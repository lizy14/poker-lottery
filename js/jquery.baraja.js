 ...
 */
 ...
 /**
 * Modified by Zhaoyang Li, SSAST, 2015.11.
...
;( function( $, window, undefined ) {
	...
	$.Baraja.prototype = {
		...
		_navigate : function( dir ) {
			...
			if(dir === 'next' && this.randRange){
				var nextZIndex = this.itemZIndexMin + this.itemsCount - 1 - 1;

				var nextCard = this.$items.filter( function() {

					var zIdx = Number( $( this ).css( 'z-index' ) ),
						cond = zIdx == nextZIndex;

					return cond;

				} )[0];

				var currentCard = this.$items.filter( function() {

					var zIdx = Number( $( this ).css( 'z-index' ) ),
						cond = zIdx == nextZIndex + 1;

					return cond;

				} )[0];

				var currentId = currentCard.firstChild.src.match(/card(\d+).jpg/)[1];

				var rand;
				do{
					rand = pickRandomly(range(this.randRange));
				}while(rand == currentId);

				nextCard.firstChild.src = 'images/card'+rand+'.jpg'
				var d = this.digitId;
				if(d < 2){
					$('#digit'+d + ' span').html(rand);
				}else{
					$('#digit3 span').html(rand >= 10? '1': '0');
					$('#digit2 span').html((rand % 10).toString());
				}
			}
		...
		_add : function( $elems ) {
			...
			$elems.css( 'transform', 'scale(1.8) translate(200px) rotate(15deg)' ).reverse().each( function( i ) {
				...
				self._setTransition( $el, 'all', 200, 'ease-out', i * 80 );
